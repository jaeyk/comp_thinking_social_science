...
Also, remember functions don't have what CS people called side-effects. Functions only define local variables = They don't change objects stored in the global environment, so on. (Consider the difference between `<-` and `=` for assignments.) That's why you can use functions for reusable tasks since it does not interrupt other important things in your system.
...
```{r}
# In fact, we could do this even faster by not using a loop at all. Instead, we can use an ifelse statement (which makes it completely vectorized, eliminating the need for a loop):
ptm <- proc.time()
