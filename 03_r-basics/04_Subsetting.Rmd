...
#### 1d.1. Slicing, selecting, and filtering

We learned the tidyverse framework last week. Let's take advantage of that to do subsetting dataframe in an easy way. Just remember slice is for selecting rows, select is for selecting columns, and filter is for selecting rows based on logical conditions.  

First, let's recap piping (%>%) for those missed the class. [The following example](https://r4ds.had.co.nz/pipes.html) by Wickham is a pseudo code. So consider that only as a thought experiment. 
...
df %>%
  slice(c(1,2)) # first 2 rows

df %>%
  select(c(1,2)) # first 2 columns 
